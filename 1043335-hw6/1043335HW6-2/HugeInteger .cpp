...
bool HugeInteger< T >::operator<(const HugeInteger< T > &right) const 
{
	T ::iterator n1 = integer.rbegin();                           // 令 n1 指向 n1 最高位
	T ::iterator n2 = right.integer.rbegin();                     // 令 n2 指向 n1 最高位
...
HugeInteger< T > HugeInteger< T >::operator+(const HugeInteger< T > &op2) const
{
	...
	for (it3 = sum.integer.begin(); it2 != op2.integer.end(); it2 = it2->llink, it3 = it3->llink)
		it3->data += it2->data;

	it3 = sum.integer.begin();
	T::iterator it4 = it3;
	for (it4 = it4->llink; it4 != sum.integer.end(); it3 = it3->llink, it4 = it4->llink)
	if (it3->data > 9)
	{
		it3->data -= 10;
		(it4->data)++;
...
{
	for (T::iterator it = integer.begin(); it != integer.end(); it = it->llink)
	if (it->data != 0)
		return false;
...
HugeInteger< T > HugeInteger< T >::operator-(const HugeInteger< T > &op2) const
{
	...
	T::iterator dif = difference.integer.begin();         // difference 的指標
	T::iterator n2 = num2.integer.begin();                // n2 的指標
...
HugeInteger< T > HugeInteger< T >::operator/(const HugeInteger< T > &op2) const
{
	...
	else
	{
		...
		T::iterator Qptr = quotient.integer.rbegin();
...
HugeInteger< T > HugeInteger< T >::operator%(const HugeInteger< T > &op2) const
{
	...
	else
	{
		...
		T::iterator Rptr = remainder.integer.rbegin();
